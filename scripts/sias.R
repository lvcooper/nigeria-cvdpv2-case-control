sias<-function(define.distance,define.age,define.onset,match.on.RI=F,match.limit=NA,under=NA,define.offset,use.date.of.last.opv=T,define.ceiling_OPV=F){

  # define.distance=50000;define.age=12;define.onset=30;match.on.RI=F;match.limit=NA;define.offset=28;use.date.of.last.opv=T;define.ceiling_OPV=T
  ...
  m1<-read.csv(paste0("data/matched_distance_",define.distance,
                      ...
                      ifelse(!is.na(under),paste0("_under_",under),""),".csv"))
...
m1$fIPV_SIAs<-sapply(1:nrow(m1),function(i){
  ...
  sub<-sia[which(sia$date>dob&
                   ...
                   sia$age<=sia$age_max&
                   sia$age>=sia$age_min),]
  sub<-unique(sub[sub$vaccine_type%in%c("f-IPV"),c("date","vaccine_type","activity_parent_code")])
  return(nrow(sub))})
...
m1$IPV_SIAs<-sapply(1:nrow(m1),function(i){
  ...
  sub<-sia[which(sia$date>dob&
                   ...
                   sia$age<=sia$age_max&
                   sia$age>=sia$age_min),]
  sub<-unique(sub[sub$vaccine_type%in%c("IPV"),c("date","vaccine_type","activity_parent_code")])
  return(nrow(sub))})
...
m1$bOPV_IPV_SIAs<-sapply(1:nrow(m1),function(i){
  ...
  sub<-sia[which(sia$date>dob&
                   ...
                   sia$age<=sia$age_max&
                   sia$age>=sia$age_min),]
  sub<-unique(sub[sub$vaccine_type%in%c("IPV + bOPV"),c("date","vaccine_type","activity_parent_code")])
  return(nrow(sub))})
...
sample_doses<-function(guid,dob,onset_date,investigation_date,date_last_opv=NA,doses,offset,ceiling_OPV=F){
  
  # i=which(m1$date==as.Date("2018-09-26")&m1$adm2=="zango")
  # dob=m1$dob[i];onset_date=m1$date[i];investigation_date=m1$investigation_date[i];guid=m1$GUID[i];date_last_opv=m1$last_opv_date[i];doses=m1$doses_opv_sia[i];offset=28
  ...
  # If ceiling option, make total doses reported cannot be more than SIAs
  if(ceiling_OPV==T&doses>nrow(exp)) doses=nrow(exp) 
...
r<-NULL
for(i in 1:nrow(m1)){
  r[[i]]<-sample_doses(dob=m1$dob[i],onset_date=m1$date[i],investigation_date=m1$investigation_date[i],guid=m1$GUID[i],date_last_opv=m1$last_opv_date[i],doses=m1$doses_opv_sia[i],offset=define.offset,ceiling_OPV=define.ceiling_OPV)
}
...
write.csv(m1,paste0("data/matched_distance_",define.distance,
                    ...
                    ifelse(match.on.RI,"_RI",""),
                    ifelse(!is.na(match.limit),paste0("_limit_",match.limit),""),
                    ifelse(!is.na(under),paste0("_under_",under),""),
                    "_offset_",define.offset,ifelse(use.date.of.last.opv,"","_no_date"),
                    ifelse(define.ceiling_OPV,"_ceiling",""),
                    ".csv"),row.names=F)}
